...
export default function CartModal({ setCartFlipper }) {
  ...
  return (
    <div className="fixed inset-0">
      ...
      ></div>
      <div className=" absolute bg-white top-0 right-0 h-full z-50 w-3/5 flex flex-col p-2 items-center px-14">
        <div className="flex w-full h-16 justify-end items-center p-2">
          <div className="relative cursor-pointer flex items-center justify-between w-24 h-8 border-2 border-gray-200 rounded-2xl px-4">
            <button
              onClick={() => {
                setCartFlipper((prev) => !prev);
              }}
              className="text-xs text-black font-bold cursor-pointer text-white rounded-2xl h-8 w-1/6 "
            >
              Close
            </button>
